...
try:
    ...
    for streamID in NODES:
        for nodeID in NODES[streamID]:
            ...
            #Add input metadata
            NODES[streamID][nodeID]["NUM_DIR"] = NumDirections
            NODES[streamID][nodeID]["NUM_ZONES"] = NumZones
            NODES[streamID][nodeID]["SAMPLE_DIS"] = TransDistance
            NODES[streamID][nodeID]["LC_RASTER"] = LCRaster
            NODES[streamID][nodeID]["ELE_RASTER"] = EleRaster
            NODES[streamID][nodeID]["CAN_RASTER"] = CanopyRaster
            NODES[streamID][nodeID]["k_RASTER"] = kRaster

            # xy string requirement for arcpy.GetCellValue
            origin_x = NODES[streamID][nodeID]["POINT_X"]
            origin_y = NODES[streamID][nodeID]["POINT_Y"]
            ...
            if StreamSample == "True":
                for t in type:
                    ...
                    if t == "ELE":
                        ...
                        NODES[streamID][nodeID][LCkey] = float(thevalue.getOutput(0)) * ele_con

                    if t in ["LC"]:
                        thevalue = arcpy.GetCellValue_management(LCRaster, xypoint)
                        NODES[streamID][nodeID][LCkey] = float(thevalue.getOutput(0))
            ...
            for a in azimuths:
                for z in zone:
                    ...
                    NODES[streamID][nodeID]["POINT_X_"+str(int(a))+'_Z'+str(z)] = _X_
                    NODES[streamID][nodeID]["POINT_Y_"+str(int(a))+'_Z'+str(z)] = _Y_			
    ...
    naz= [[s,n,a,z] for s in NODES for n in NODES[s] for a in azimuths for z in zone ]
    ...
    LCpointlist = [naz[row] + [NODES[naz[row][0]][naz[row][1]][t+'_'+str(int(naz[row][2]))+'_Z'+str(naz[row][3])] for t in type2] for row in range(0,len(naz))]
    ...
    if StreamSample == "True":
        ...
        naz_e= [[s,n,a,z] for s in NODES for n in NODES[s] for a in [0] for z in [0]]
        #LCpointlist = LCpointlist + [naz_e[row] + [NODES[naz_e[row][0]][t] for t in type_emergent] for row in range(0,len(naz_e))]
        LCpointlist = LCpointlist + [naz_e[row] + [NODES[naz[row][0]][naz_e[row][1]][t] for t in type_emergent] for row in range(0,len(naz_e))]
