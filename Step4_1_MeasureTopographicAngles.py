...
def CalculateTopoAngles(pointfile,EleRaster,MaxSearchDistance,nodexy_to_m,Elexy_to_m,units_con):
    """Reads the input point file, and calculates the topo angles and other information returned as a nested dictionary and list"""
    NODES = NestedDictTree()
    TOPO_fc = []
    Incursorfields = ["NODE_ID", "SHAPE@X","SHAPE@Y"]
    n = 0
    # Determine input point spatial units
    proj = arcpy.Describe(inPoint).spatialReference
    nTotal = int(arcpy.GetCount_management(pointfile).getOutput(0))
    elapsedNodemin = 0.7
    with arcpy.da.SearchCursor(pointfile,Incursorfields,"",proj) as Inrows:
	for row in Inrows:
	    print("Processing Node %s of %s. %s minutes remaining" % (n+1, nTotal, elapsedNodemin * (nTotal - n)))
	    #keeping track of time to process each node
	    startNodeTime= time.time()	    
	    # Set the progressor
	    arcpy.SetProgressor("step", "",0, nTotal, 1)
	    arcpy.SetProgressorLabel("Processing Node %s of %s. %s minutes remaining" % (n+1, nTotal, elapsedNodemin * (nTotal - n)))
	    #arcpy.AddMessage("Processing Node %s of %s. %s minutes remaining" % (n, nTotal, elapsedNodemin))	    
	    node_x = row[1]
	    node_y = row[2]
		...
		TOPO_fc.append([row[0], azimuthdict[azimuths[a]], MaxShadeAngle, SampleZFinal, MaxZChange,FinalSearchDistance, MaxSearchDistance * nodexy_to_m, offRasterSamples, MaxShadeAngle_X, MaxShadeAngle_Y, MaxShadeAngle_X,MaxShadeAngle_Y])
		NODES[row[0]]["TOPO_"+ str(azimuthdict[azimuths[a]])]= MaxShadeAngle
		endNodeTime = time.time()
		elapsedNodemin = ceil(((endNodeTime - startNodeTime) / 60)* 10)/10
	    arcpy.SetProgressorPosition()
	    n = n + 1
    return(NODES,TOPO_fc)
...
try:
    ...
    # Calculate the topo angles and add to a nested dictionary
    NODES, TOPO_fc = CalculateTopoAngles(inPoint,EleRaster,MaxSearchDistance,nodexy_to_m,Elexy_to_m,units_con)			
    gc.collect()
